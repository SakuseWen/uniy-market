...
const ReportModel_1 = require("../../src/models/ReportModel");
const AuditLogModel_1 = require("../../src/models/AuditLogModel");
...
describe('Admin System', () => {
    ...
    let testCounter = 0;
    beforeAll(async () => {
        dbManager = database_1.DatabaseManager.getInstance();
        await dbManager.initialize();
        userModel = new UserModel_1.UserModel();
        productModel = new ProductModel_1.ProductModel();
        reportModel = new ReportModel_1.ReportModel();
        auditLogModel = new AuditLogModel_1.AuditLogModel();
        adminService = new AdminService_1.AdminService();
    });
    beforeEach(async () => {
        testCounter++;
    ...
    describe('AdminService - User Management', () => {
        ...
        it('should not suspend already suspended user', async () => {
            ...
            await expect(adminService.suspendUser(adminUser.userID, regularUser.userID)).rejects.toThrow('User is already suspended');
        });
        ...
        it('should not activate already active user', async () => {
            await expect(adminService.activateUser(adminUser.userID, regularUser.userID)).rejects.toThrow('User is already active');
        });
        ...
        it('should not delete admin user', async () => {
            await expect(adminService.deleteUser(adminUser.userID, adminUser.userID)).rejects.toThrow('Cannot delete admin users');
        });
        ...
        it('should not grant admin to already admin user', async () => {
            ...
            await expect(adminService.makeAdmin(adminUser.userID, regularUser.userID)).rejects.toThrow('User is already an admin');
        });
        ...
        it('should not revoke admin from non-admin user', async () => {
            await expect(adminService.removeAdmin(adminUser.userID, regularUser.userID)).rejects.toThrow('User is not an admin');
        });
        it('should not allow admin to remove own admin privileges', async () => {
            await expect(adminService.removeAdmin(adminUser.userID, adminUser.userID)).rejects.toThrow('Cannot remove your own admin privileges');
        });
    });
    describe('AdminService - Content Moderation', () => {
        it('should remove product', async () => {
            await adminService.removeProduct(adminUser.userID, testProduct.listingID, 'Test removal');
        ...
        it('should restore product', async () => {
            ...
            await adminService.restoreProduct(adminUser.userID, testProduct.listingID, 'Test restore');
            const product = await productModel.getProductById(testProduct.listingID);
        ...
        it('should throw error when removing non-existent product', async () => {
            await expect(adminService.removeProduct(adminUser.userID, 'non-existent-id')).rejects.toThrow('Product not found');
        });
        it('should throw error when restoring non-existent product', async () => {
            await expect(adminService.restoreProduct(adminUser.userID, 'non-existent-id')).rejects.toThrow('Product not found');
        });
    });
    describe('AdminService - Report Management', () => {
        ...
        it('should resolve report', async () => {
            await adminService.resolveReport(adminUser.userID, testReport.report_id, 'Test resolution');
        ...
        it('should not resolve already resolved report', async () => {
            ...
            await expect(adminService.resolveReport(adminUser.userID, testReport.report_id)).rejects.toThrow('Report is already resolved');
        });
        it('should dismiss report', async () => {
            await adminService.dismissReport(adminUser.userID, testReport.report_id, 'Test dismissal');
            const report = await reportModel.findById(testReport.report_id);
        ...
        it('should not dismiss already dismissed report', async () => {
            ...
            await expect(adminService.dismissReport(adminUser.userID, testReport.report_id)).rejects.toThrow('Report is already dismissed');
        });
        it('should throw error when resolving non-existent report', async () => {
            await expect(adminService.resolveReport(adminUser.userID, 99999)).rejects.toThrow('Report not found');
        });
        it('should throw error when dismissing non-existent report', async () => {
            await expect(adminService.dismissReport(adminUser.userID, 99999)).rejects.toThrow('Report not found');
        });
    });
