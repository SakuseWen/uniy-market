...
describe('Product CRUD API Endpoints', () => {
    ...
    beforeAll(async () => {
        ...
        const categoryResult = await db.run('INSERT INTO Category (name, nameEn, nameTh, nameZh, isActive) VALUES (?, ?, ?, ?, ?)', ['Electronics', 'Electronics', 'อิเล็กทรอนิกส์', '电子产品', 1]);
    ...
    describe('GET /api/products (Search)', () => {
        ...
        it('should filter products by price range', async () => {
            ...
            expect(response.body.success).toBe(true);
            expect(response.body.data.data).toBeDefined();
            response.body.data.data.forEach((product) => {
