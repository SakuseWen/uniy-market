...
describe('Product Status Management - Task 4.7', () => {
    ...
    beforeEach(async () => {
        ...
        await NotificationService_1.notificationService.clearUserNotifications(testUser.userID);
    });
    describe('Mark as Sold Functionality', () => {
        ...
        test('should update updatedAt timestamp when marking as sold', async () => {
            ...
            expect(new Date(soldProduct.updatedAt).getTime()).toBeGreaterThan(new Date(originalUpdatedAt).getTime());
        });
    });
    describe('Status Change Notifications', () => {
        test('should create notification when product is marked as sold', async () => {
            ...
            // Check notifications
            const notifications = await NotificationService_1.notificationService.getUserNotifications(testUser.userID);
    ...
    describe('Notification Service Operations', () => {
        ...
        test('should get unread notifications count', async () => {
            await productModel.markAsSold(testProduct.listingID);
            await new Promise(resolve => setTimeout(resolve, 50));
            const count = await NotificationService_1.notificationService.getNotificationCount(testUser.userID);
        ...
        test('should mark notification as read', async () => {
            ...
            const success = await NotificationService_1.notificationService.markAsRead(testUser.userID, notificationId);
            expect(success).toBe(true);
            const updatedNotifications = await NotificationService_1.notificationService.getUserNotifications(testUser.userID);
        ...
        test('should mark all notifications as read', async () => {
            ...
            await productModel.updateProduct(product2.listingID, { status: 'inactive' });
            await new Promise(resolve => setTimeout(resolve, 50));
            const markedCount = await NotificationService_1.notificationService.markAllAsRead(testUser.userID);
        ...
        test('should get only unread notifications', async () => {
            await productModel.markAsSold(testProduct.listingID);
            await new Promise(resolve => setTimeout(resolve, 50));
            const allNotifications = await NotificationService_1.notificationService.getUserNotifications(testUser.userID);
            expect(allNotifications[0]).toBeDefined();
            await NotificationService_1.notificationService.markAsRead(testUser.userID, allNotifications[0].notificationID);
            const unreadNotifications = await NotificationService_1.notificationService.getUnreadNotifications(testUser.userID);
            expect(unreadNotifications.length).toBe(0);
        });
    });
    describe('Edge Cases', () => {
        test('should handle marking non-existent product as sold', async () => {
            await expect(productModel.markAsSold('non_existent_id')).rejects.toThrow();
        });
