...
describe('Chat Message Translation Integration', () => {
    ...
    describe('Translation Service Integration', () => {
        ...
        test.skip('should detect language of text', async () => {
            const detection = await TranslationService_1.default.detectLanguage('Hello, how are you?');
            expect(detection).toBeDefined();
            expect(detection.language).toBeDefined();
            expect(detection.confidence).toBeGreaterThanOrEqual(0);
        });
        test.skip('should translate text between languages', async () => {
            const translation = await TranslationService_1.default.translateText('Hello', 'th', 'en');
        ...
        test.skip('should use cache for repeated translations', async () => {
            ...
            // First translation
            const translation1 = await TranslationService_1.default.translateText(text, 'th', 'en');
            // Second translation (should use cache)
            const translation2 = await TranslationService_1.default.translateText(text, 'th', 'en');
        ...
        test('should handle translation errors gracefully', async () => {
            await expect(TranslationService_1.default.translateText('', 'th', 'en')).rejects.toThrow('Text to translate cannot be empty');
        });
        test.skip('should translate batch of texts', async () => {
            const texts = ['Hello', 'Goodbye', 'Thank you'];
            const translations = await TranslationService_1.default.translateBatch(texts, 'th', 'en');
    ...
    describe('Automatic Translation Workflow', () => {
        test.skip('should create message with automatic translation based on user preference', async () => {
            ...
            // Detect language
            const detection = await TranslationService_1.default.detectLanguage(originalText);
            // Translate to recipient's language
            const translation = await TranslationService_1.default.translateText(originalText, testUser2.preferredLanguage, detection.language);
        ...
        test.skip('should handle multiple messages with different translations', async () => {
            ...
            for (const msg of messages) {
                const translation = await TranslationService_1.default.translateText(msg.text, 'en', msg.lang);
                await messageModel.createMessage({
                    chatID: testChat.chatID,
                    senderID: testUser1.userID,
    ...
    describe('Translation Cache Management', () => {
        test.skip('should cache translations', async () => {
            ...
            // First translation
            await TranslationService_1.default.translateText(text, 'th', 'en');
            const stats = TranslationService_1.default.getCacheStats();
            expect(stats.size).toBeGreaterThan(0);
        });
        test.skip('should clear cache', async () => {
            await TranslationService_1.default.translateText('Test', 'th', 'en');
            TranslationService_1.default.clearCache();
            const stats = TranslationService_1.default.getCacheStats();
