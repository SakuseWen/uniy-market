...
const LocationService_1 = require("../../src/services/LocationService");
describe('LocationService', () => {
    ...
    describe('generateLocationShareMessage', () => {
        ...
        it('should include additional info when provided', () => {
            const message = locationService.generateLocationShareMessage('Near Campus', 'Behind the library');
            expect(message).toContain('Near Campus');
    ...
    describe('validateLocationUpdate', () => {
        it('should allow valid location updates', () => {
            const result = locationService.validateLocationUpdate('Near Campus', 'University District');
            expect(result.valid).toBe(true);
        });
        it('should reject invalid new locations', () => {
            const result = locationService.validateLocationUpdate('Near Campus', '13.7563, 100.5018');
            expect(result.valid).toBe(false);
        });
        it('should reject updates that make location more specific', () => {
            const result = locationService.validateLocationUpdate('Near Campus', 'Near Campus, Building A, Room 301, Floor 3, Behind the library');
            expect(result.valid).toBe(false);
            expect(result.reason).toContain('specific');
        });
        it('should allow updates from null', () => {
            const result = locationService.validateLocationUpdate(null, 'Near Campus');
            expect(result.valid).toBe(true);
        });
    });
