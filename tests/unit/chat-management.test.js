...
const ChatModel_1 = require("../../src/models/ChatModel");
...
describe('Chat Management Features', () => {
    ...
    beforeAll(async () => {
        ...
        // Initialize models
        chatModel = new ChatModel_1.ChatModel();
    ...
    beforeEach(async () => {
        // Clear tables
        const db = database_1.DatabaseManager.getInstance().getDatabase();
    ...
    describe('Message Read Status Tracking', () => {
        ...
        test('should mark all chat messages as read for specific user', async () => {
            ...
            const updatedCount = await messageModel.markChatMessagesAsRead(testChat.chatID, testUser1.userID);
    ...
    describe('Real-Time Notification System', () => {
        test('should create new message notification', async () => {
            const notification = await NotificationService_1.notificationService.createNewMessageNotification(testUser1.userID, testChat.chatID, testUser2.userID, testUser2.name, 'Hello, is this still available?', testProduct.title);
        ...
        test('should create chat deleted notification', async () => {
            const notification = await NotificationService_1.notificationService.createChatDeletedNotification(testUser1.userID, testChat.chatID, testUser2.userID, testProduct.title);
            expect(notification).toBeDefined();
        ...
        test('should create messages read notification', async () => {
            const notification = await NotificationService_1.notificationService.createMessagesReadNotification(testUser2.userID, testChat.chatID, testUser1.name, 3);
            expect(notification).toBeDefined();
        ...
        test('should get all chat notifications for user', async () => {
            ...
            await NotificationService_1.notificationService.createNewMessageNotification(testUser1.userID, testChat.chatID, testUser2.userID, testUser2.name, 'Message 1', testProduct.title);
            await NotificationService_1.notificationService.createChatDeletedNotification(testUser1.userID, testChat.chatID, testUser2.userID, testProduct.title);
            const notifications = await NotificationService_1.notificationService.getChatNotifications(testUser1.userID);
            expect(notifications).toHaveLength(2);
            expect(notifications.every(n => 'chatID' in n)).toBe(true);
        });
        test('should get unread chat notifications count', async () => {
            await NotificationService_1.notificationService.createNewMessageNotification(testUser1.userID, testChat.chatID, testUser2.userID, testUser2.name, 'Unread message', testProduct.title);
            const count = await NotificationService_1.notificationService.getUnreadChatNotificationsCount(testUser1.userID);
            expect(count).toBe(1);
        });
        test('should mark notification as read', async () => {
            const notification = await NotificationService_1.notificationService.createNewMessageNotification(testUser1.userID, testChat.chatID, testUser2.userID, testUser2.name, 'Test message', testProduct.title);
            const success = await NotificationService_1.notificationService.markAsRead(testUser1.userID, notification.notificationID);
            expect(success).toBe(true);
            const notifications = await NotificationService_1.notificationService.getUnreadNotifications(testUser1.userID);
        ...
        test('should truncate long message previews', async () => {
            ...
            const notification = await NotificationService_1.notificationService.createNewMessageNotification(testUser1.userID, testChat.chatID, testUser2.userID, testUser2.name, longMessage, testProduct.title);
