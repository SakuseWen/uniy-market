...
const ContentModerationService_1 = require("../../src/services/ContentModerationService");
describe('ContentModerationService', () => {
    let service;
    beforeEach(() => {
        service = new ContentModerationService_1.ContentModerationService();
    });
    ...
    describe('moderateProductListing', () => {
        it('should approve clean product listing', () => {
            const result = service.moderateProductListing('Used Textbook', 'Great condition, barely used');
        ...
        it('should reject product with profanity in title', () => {
            const result = service.moderateProductListing('Fucking awesome book', 'Great condition');
            expect(result.approved).toBe(false);
            expect(result.titleResult.shouldBlock).toBe(true);
            expect(result.reason).toContain('Title');
        });
        it('should reject product with profanity in description', () => {
            const result = service.moderateProductListing('Used Book', 'This shit is amazing');
            expect(result.approved).toBe(false);
            expect(result.descriptionResult.flagged).toBe(true);
        });
        it('should reject spam product', () => {
            const result = service.moderateProductListing('BUYNOWBUYNOWBUYNOW', 'Click here!!!!!!');
            expect(result.approved).toBe(false);
            expect(result.spamCheck.isSpam).toBe(true);
        });
        it('should detect fraud keywords', () => {
            const result = service.moderateProductListing('Stolen iPhone', 'Counterfeit designer bag');
            expect(result.approved).toBe(false);
    ...
    describe('Edge cases', () => {
        it('should handle null/undefined gracefully', () => {
            const result = service.moderateContent(null);
            expect(result.isClean).toBe(true);
        });
