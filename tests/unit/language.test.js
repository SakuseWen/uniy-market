...
const LocalizationService_1 = __importDefault(require("../../src/services/LocalizationService"));
describe('Language Switching / 语言切换', () => {
    ...
    describe('LocalizationService', () => {
        it('should get string from English locale', () => {
            const str = LocalizationService_1.default.getString('en', 'common.welcome');
            expect(str).toBe('Welcome');
        });
        it('should get string from Thai locale', () => {
            const str = LocalizationService_1.default.getString('th', 'common.welcome');
            expect(str).toBe('ยินดีต้อนรับ');
        });
        it('should get string from Chinese locale', () => {
            const str = LocalizationService_1.default.getString('zh', 'common.welcome');
            expect(str).toBe('欢迎');
        });
        it('should return fallback for missing key', () => {
            const str = LocalizationService_1.default.getString('en', 'nonexistent.key', 'Fallback');
            expect(str).toBe('Fallback');
        });
        it('should return key if no fallback provided', () => {
            const str = LocalizationService_1.default.getString('en', 'nonexistent.key');
            expect(str).toBe('nonexistent.key');
        });
        it('should get entire section', () => {
            const section = LocalizationService_1.default.getSection('en', 'common');
            expect(section).toBeDefined();
            expect(section?.['welcome']).toBe('Welcome');
            expect(section?.['login']).toBe('Login');
        });
        it('should check if language is supported', () => {
            expect(LocalizationService_1.default.isLanguageSupported('en')).toBe(true);
            expect(LocalizationService_1.default.isLanguageSupported('th')).toBe(true);
            expect(LocalizationService_1.default.isLanguageSupported('zh')).toBe(true);
            expect(LocalizationService_1.default.isLanguageSupported('fr')).toBe(false);
        });
        it('should get list of supported languages', () => {
            const languages = LocalizationService_1.default.getSupportedLanguages();
    ...
    describe('Real-time Language Switching', () => {
        it('should immediately reflect language change in user profile', async () => {
            ...
            const profileResponse = await (0, supertest_1.default)(index_1.app)
    ...
    describe('Localization Coverage', () => {
        ...
        languages.forEach(lang => {
            it(`should have all required sections for ${lang}`, () => {
                const localeData = LocalizationService_1.default.getAllStrings(lang);
        ...
        it('should have consistent keys across all languages', () => {
            const enData = LocalizationService_1.default.getAllStrings('en');
            const thData = LocalizationService_1.default.getAllStrings('th');
            const zhData = LocalizationService_1.default.getAllStrings('zh');
