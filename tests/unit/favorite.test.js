...
const FavoriteModel_1 = require("../../src/models/FavoriteModel");
...
describe('FavoriteModel', () => {
    ...
    beforeAll(async () => {
        ...
        favoriteModel = new FavoriteModel_1.FavoriteModel();
        db = database_1.DatabaseManager.getInstance().getDatabase();
        // Create test users
        await db.run(`
    ...
    async function createTestListing(listingID, title) {
        await db.run(`
      INSERT OR IGNORE INTO ProductListing (listingID, title, description, price, categoryID, sellerID, status)
      VALUES (?, ?, ?, ?, ?, ?, ?)
