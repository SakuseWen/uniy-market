...
describe('DatabaseManager', () => {
    ...
    describe('Singleton Pattern', () => {
        it('should return the same instance', () => {
            const instance1 = database_1.DatabaseManager.getInstance();
            const instance2 = database_1.DatabaseManager.getInstance();
    ...
    describe('Database Schema Integrity', () => {
        ...
        it('should have proper foreign key constraints', async () => {
            ...
            await expect(db.run('INSERT INTO Student (studentID, userID, schoolName, studentEmail) VALUES (?, ?, ?, ?)', ['test-student', 'non-existent-user', 'Test School', 'test@test.com'])).rejects.toThrow();
        });
        it('should have proper indexes created', async () => {
            const db = dbManager.getDatabase();
            // Query to get all indexes
            const indexes = await db.all("SELECT name FROM sqlite_master WHERE type='index' AND name NOT LIKE 'sqlite_%'");
    ...
    describe('Database Operations', () => {
        ...
        it('should throw error when getting database before initialization', () => {
            const freshManager = Object.create(database_1.DatabaseManager.prototype);
            expect(() => freshManager.getDatabase()).toThrow('Database not initialized. Call initialize() first.');
        });
        it('should verify database schema exists', async () => {
            const db = dbManager.getDatabase();
            // Check if User table exists
            const userTable = await db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='User'");
            expect(userTable).toBeDefined();
            expect(userTable.name).toBe('User');
            // Check if Category table exists
            const categoryTable = await db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='Category'");
