...
const ReviewModel_1 = require("../../src/models/ReviewModel");
...
describe('ReviewModel', () => {
    ...
    beforeAll(async () => {
        ...
        reviewModel = new ReviewModel_1.ReviewModel();
    ...
    describe('createReview', () => {
        ...
        it('should reject rating below 1', async () => {
            ...
            await expect(reviewModel.createReview(reviewData)).rejects.toThrow('Rating must be between 1 and 5');
        });
        ...
        it('should reject invalid review type', async () => {
            ...
            await expect(reviewModel.createReview(reviewData)).rejects.toThrow('Invalid review type');
        });
        it('should reject self-review', async () => {
            ...
            await expect(reviewModel.createReview(reviewData)).rejects.toThrow('Cannot review yourself');
        });
    });
    ...
    describe('getReviewsByTargetUser', () => {
        ...
        it('should filter reviews by type', async () => {
            const buyerReviews = await reviewModel.getReviewsByTargetUser(testUser2.userID, 'seller_to_buyer');
            expect(buyerReviews.every((r) => r.reviewType === 'seller_to_buyer')).toBe(true);
            const sellerReviews = await reviewModel.getReviewsByTargetUser(testUser2.userID, 'buyer_to_seller');
            expect(sellerReviews.every((r) => r.reviewType === 'buyer_to_seller')).toBe(true);
        });
    });
    describe('getReviewsByReviewer', () => {
        it('should get all reviews written by a user', async () => {
            ...
            expect(reviews.length).toBeGreaterThan(0);
            expect(reviews.every((r) => r.reviewerID === testUser1.userID)).toBe(true);
        });
    });
